...
/* Project1 
 * 
 * This program filters out the total rainfall for each
 * zip code with it's corresponding year starting from 1950.
 * 
 * Date@ Feb-09,2021
...
public class TotalRainfall extends Configured implements Tool {
 		...
 		public void map(LongWritable inputKey, Text inputValue, Context context) throws IOException, InterruptedException
 		{
 			...
 			for(int i = 0; i < element.length; i++)
 			{
	 			if(st.hasMoreTokens())
	 			{
	 				...
	 				if(i == 7 && Integer.parseInt(element[i]) < 1950)
						return;
	 			}
				else 
					return;
 			...
 			context.write(new Text(element[ZIPCODE]+"\t"+new Text(element[YEAR])), new DoubleWritable(Double.parseDouble(element[PERCIP])));
	...
	public static class MyReducer extends Reducer<Text, DoubleWritable, Text, Text>
	{
		...
		{
			context.write(new Text("Zipcode\t" + "Year\t"+ "Total-Rainfall\t"+"\n"),null);
		}
	...
	public int run(String[] args) throws Exception 
	{
		if(args.length != 2)
		{
			System.out.println("Usage: bin/hadoop jar InstructionalSolutions.jar TotalRainfall <input directory> <ouput directory> <number of reduces>");
		...
		Job job = new Job(conf, "TotalRainfall Test");
